 ...
 * mode. The flows that touch Privy auth or Convex writes (full signup create,
 * real lead insert) need live config; those specs are tagged `@needs-creds` and
 * are skipped automatically when the env is absent (see tests/e2e/_env.ts), so the
